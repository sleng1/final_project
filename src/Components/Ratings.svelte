<script>
  ...
  async function loadData() {
    data = await d3.csv('ratings_data.csv');
  ...
  function drawChart(data) {
    ...
    const r = d3.scaleSqrt()
      .domain([0, d3.max(data, d => +d.MIN_x)])
      .range([0, 20]);
    ...
    svg.append('text')
      ...
      .text('Analyzing Players\' Offensive And Defensive Ratings');
    ...
    const tooltip = d3.select("#ratings-chart").append("div")
    .attr("class", "tooltip")
    .style("opacity", 0)
  ...
  const legendSize = [d3.min(data, d => +d.MIN_x), d3.median(data, d => +d.MIN_x), d3.max(data, d => +d.MIN_x)];
  ...
  legend.append('text')
    ...
    .text('Bubble Size: Minutes Per Game');
...
<main>
<body>
  <label for="team-select-ratings">Select Team:</label>
  <select id="team-select-ratings" bind:value={selectedTeam} on:change={updateChart}>
    {#each teams as team}
      <option value={team}>{team}</option>
    {/each}
  </select>
  <svg id="ratings-chart"></svg>
